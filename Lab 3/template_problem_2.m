...
% Bounds
pc_bound = 60*pi/360;
ul 	    = -pc_bound * ones(mu,1);                   % Lower bound on control
uu 	    = pc_bound * ones(mu,1);                   % Upper bound on control
...
tic
[z,lambda] = quadprog(Q,c,[],[],Aeq,beq,vlb,vub,x0,options); % hint: quadprog. Type 'doc quadprog' for more info 
t1=toc;
...
x_optimum = [x1 x2 x3 x4];

%%LQR
Q_lqr = diag([1 1 1 1]);
R_lqr = 1;

[K,p,e] = dlqr(A1,B1,Q_lqr,R_lqr);
%% Plotting
